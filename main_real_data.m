...
%     end
    ...
    % k-means
    fprintf('k-means:\n');
    [beta_kmns, alpha_kmns, theta_kmns, subgroup_kmns, ~, timecost_kmns] = kmeans(X, Z, Y, train_idx, valid_idx, theta_U, W);
    S_kmns = size(subgroup_kmns,2);
    ...
    error_kmns = pred_err(X, Z, Y, beta_kmns, theta_kmns, test_idx);
    fprintf('S: %d, error: %.4f.', S_kmns, error_kmns);
